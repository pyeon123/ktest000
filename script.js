...
// ===== AI Tutor - ONLY where "Your Korean Learning Progress" exists =====
document.addEventListener('DOMContentLoaded', function() {
  try {
    const style = document.createElement('style');
    style.textContent = `
      #ai-tutor-btn{display:none!important;position:fixed!important;bottom:90px!important;right:18px!important;width:60px!important;height:60px!important;border-radius:50%!important;background:#4f46e5!important;color:white!important;border:2px solid #3730a3!important;border-bottom-width:4px!important;font-size:1.6rem!important;cursor:pointer!important;z-index:9999!important;box-shadow:0 6px 16px rgba(79,70,229,0.35)!important;align-items:center!important;justify-content:center!important;}
    ...
    div.innerHTML = `
      ...
      <div id="ai-tutor-modal">
        ...
        <div id="ai-chat-log" style="flex:1;overflow-y:auto;padding:12px;font-size:0.9rem;display:flex;flex-direction:column;gap:8px;"></div>
    ...
    document.getElementById('ai-tutor-close').addEventListener('click', toggle);
    ...
    function renderTutor() {
      ...
      faqBox.innerHTML = faqs.map(q=>`<button class="faq-chip" data-q="${q.replace(/"/g,'&quot;')}">${q}</button>`).join('');
      log.innerHTML = `<div style="background:#f5f3ff;padding:10px 12px;border-radius:12px;"><b>AI:</b> You got <b style="color:#4f46e5;">"${ctx.kr}"</b>! Ask me anything 👋</div>`;
      
      div.querySelectorAll('.faq-chip').forEach(c=>{
        c.addEventListener('click', ()=> sendQ(c.dataset.q));
      });
    }
    ...
    function sendQ(q) {
      ...
      const ctx = getCtx();
      setTimeout(()=>{
        let ans = q.includes('요') ? `"요" = polite! ${ctx.kr.replace('요','')} → ${ctx.kr}` : `"${ctx.kr}" = ${ctx.en}<br>Ex: ${ctx.kr} 정말 좋아요!`;
    ...
    // 핵심: "Your Korean Learning Progress" 텍스트가 있는 페이지에서만 버튼 표시
    function checkProgressPage() {
      const pageText = document.body.innerText || '';
      const hasProgress = pageText.includes('Your Korean Learning Progress') || 
                         pageText.includes('korean learning progress') ||
                         pageText.includes('Learning Progress') ||
                         document.querySelector('[class*="progress"]') && document.querySelector('[class*="progress"]')?.innerText.includes('Progress');

      // pathname이 /phrases/ 인 경우도 포함 (개별 200개 페이지)
      const isIndividualPage = location.pathname.includes('/phrases/') || location.pathname.includes('/quiz/');

      if (hasProgress || (isIndividualPage && document.querySelector('.example-box'))) {
        // 정답 화면이 실제로 보이는지 한 번 더 체크 (correct 관련 요소)
        const correctVisible = document.querySelector('.example-box')?.offsetParent !== null || 
                              document.querySelector('.related-container')?.offsetParent !== null ||
                              hasProgress;
        if (correctVisible) {
    ...
    // 계속 감시
    setInterval(checkProgressPage, 800);
    new MutationObserver(checkProgressPage).observe(document.body, { childList: true, subtree: true });
    checkProgressPage();

  } catch(e) { console.log('Tutor error', e); }
});
